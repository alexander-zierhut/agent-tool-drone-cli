...
def test_report_runs_offline_and_names_this_repo(monkeypatch, tmp_path):
    monkeypatch.setenv("DRONECLI_CONFIG_DIR", str(tmp_path / "does-not-exist"))
    ...
    assert data["repo"] == "alexander-zierhut/agent-tool-drone-cli"
    assert data["published"] is True
    assert data["version"] == __version__
    assert data["issues"].endswith("/agent-tool-drone-cli/issues")
