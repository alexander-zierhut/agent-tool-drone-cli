...
class AppContext:
    ...
    def __init__(
        ...
        *,
        ...
        self.emitter = Emitter(
            ...
            stream=os.environ.get("DRONECLI_STREAM") == "1",
        )
    ...
    def _resolve_format(self, explicit: OutputFormat | None) -> OutputFormat:
        """Precedence: --format/-o > $DRONECLI_FORMAT > saved default > ask once > json.

        NOTE the explicit rung does NOT swallow its own errors. OpenProject wraps
        the whole chain in `except ValueError: pass`, so `-o /tmp/x.pdf` silently
        degraded to json instead of complaining — which is how a wrong-file write
        shipped for four releases. A bad --format is a usage error; say so.
        """
        if explicit is not None:
            return explicit
        cli_fmt = os.environ.get("DRONECLI_CLI_FORMAT")
        if cli_fmt:
    ...
    def _ask_default_format(self) -> OutputFormat | None:
        ...
        try:
            ...
            self.config.save()
            sys.stderr.write(f"Saved. Change it any time: `drone-cli settings set-format <fmt>`\n\n")
        except Exception:
    ...
    def _maybe_offer_claude_skill(self) -> None:
        ...
        Without this, `drone-cli install claude` is discoverable only by someone
        who already read the help far enough to know it exists — i.e. the person
        who least needs telling. The skill is what makes Claude reach for this CLI
        at all, so a user who never learns it exists gets none of the tool.
        ...
        * Everything is swallowed. A nicety must never be the reason a real
        ...
        try:
            ...
            sys.stderr.write(
                "\nClaude Code is installed here. Register `drone-cli` as a skill, so Claude\n"
                "uses it automatically when you mention Drone CI?\n"
                "  writes ~/.claude/skills/drone-ci/SKILL.md — undo with "
                "`drone-cli install claude --uninstall`\n"
                "Install it? [y/N]: "
            )
            sys.stderr.flush()
            ans = (sys.stdin.readline() or "").strip().lower()
            if ans not in ("y", "yes"):
                ...
                sys.stderr.write(
                    ...
                    "Change your mind any time: `drone-cli install claude`\n\n"
                )
                return
    ...
    def _resolve_fields() -> list[str] | None:
        raw = os.environ.get("DRONECLI_CLI_FIELDS")
        if not raw:
            return None
    ...
    def client(self) -> Client:
        if self._client is None:
            ...
            if not token:
                raise ConfigError(
                    "no API token. Run `drone-cli auth login`, or set DRONE_TOKEN "
                    f"(get one at {prof.base_url.rstrip('/')}/account)."
                )
            self._client = Client(
                prof.base_url,
                token,
                verify_ssl=prof.verify_ssl,
                dry_run=os.environ.get("DRONECLI_DRY_RUN") == "1",
                user_agent=f"agent-tool-drone-cli/{__version__}",
            )
        return self._client
